                    ...
                    <div className="grid lg:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 flex flex-col">
                            ...
                            <div className="flex-grow flex flex-col justify-center space-y-1">
                                <StatRow label="구독자" valA={formatNumber(result.statsA.subscribers)} valB={formatNumber(result.statsB.subscribers)} />
                                <StatRow label="총 조회수" valA={formatNumber(result.statsA.totalViews)} valB={formatNumber(result.statsB.totalViews)} />
                                <StatRow label="평균 조회수" valA={formatNumber(result.statsA.avgViews)} valB={formatNumber(result.statsB.avgViews)} />
                                <StatRow label="참여율" valA={result.statsA.engagementRate} valB={result.statsB.engagementRate} unit="%" />
                                <StatRow label="월 업로드" valA={result.statsA.uploadFrequency} valB={result.statsB.uploadFrequency} unit="개" />
                                <StatRow label="총 영상" valA={result.statsA.videoCount} valB={result.statsB.videoCount} unit="개" />
                            </div>
                        </div>
                    </div>
                </div>
