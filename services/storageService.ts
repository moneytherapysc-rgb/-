...
export const deleteFolder = (id: string) => {
    ...
    // Move favorites in this folder to 'uncategorized' (or just delete them? Let's delete for now or unassign)
    // Actually better to keep them but with invalid folderId, or user can manage.
    // Let's delete favorites associated with deleted folder for simplicity
...
export const getFavorites = (folderId?: string): FavoriteItem[] => {
    ...
    const allFavorites: FavoriteItem[] = stored ? JSON.parse(stored) : [];
...
export const getNotices = (): Notice[] => {
    ...
    if (!stored) {
        ...
        const dummy: Notice[] = [
            ...
            {
                id: 'notice-2',
                title: '🚀 [신규 기능] AI 보이스/이미지 스튜디오 & 캘린더 업데이트',
                content: '유메이커의 강력한 크리에이티브 도구 3종이 업데이트 되었습니다!\n\n1. AI 이미지 스튜디오: 텍스트만으로 고화질 이미지를 생성하고 편집하세요.\n2. AI 보이스 스튜디오: 대본을 입력하면 전문 성우급 내레이션을 만들어줍니다.\n3. 트렌드 예측 캘린더: 다가올 시즌 이슈를 미리 파악하고 촬영 스케줄을 AI가 기획해줍니다.\n\n지금 바로 "크리에이티브 도구" 메뉴에서 확인해보세요!',
                createdAt: now,
                author: '관리자'
            },
            {
            ...
            {
                id: 'notice-4',
                title: '💡 유메이커 200% 활용하는 꿀팁',
                content: '1. [떡상 영상 분석]에서 "최소 조회수" 필터를 걸어 내 체급에 맞는 벤치마킹 영상을 찾으세요.\n2. [쇼츠 아이디어]에서 생성된 대본을 [AI 보이스 스튜디오]로 가져가면 1분 만에 오디오까지 완성됩니다.\n3. [마이 라이브러리]에 폴더를 만들어 경쟁 채널과 아이디어 영상을 체계적으로 스크랩하세요.',
                createdAt: now,
                author: '관리자'
            },
            {
        ...
        localStorage.setItem(NOTICES_KEY, JSON.stringify(dummy));
        return dummy;
    }
