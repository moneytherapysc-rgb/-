...
import { subscriptionPlans, requestPayment, redeemCoupon } from '../services/subscriptionService';
...
const PricingModal: React.FC<PricingModalProps> = ({ isOpen, onClose, onOpenLoginModal }) => {
  ...
  const eventPlan = subscriptionPlans.find(p => p.id === 'event_launch');
  const standardPlans = subscriptionPlans.filter(p => p.id !== 'event_launch');

  const handlePayment = async (planId: string | null = selectedPlan) => {
    if (!user) {
        onClose();
        onOpenLoginModal();
        return;
    ...
    if (!planId) {
        setMessage({ type: 'error', text: '플랜을 선택해주세요.' });
        return;
    }
    ...
    try {
        // Use the new requestPayment function which triggers PortOne SDK
                        ...
                        {standardPlans.map((plan) => {
                            ...
                            return (
                                <div 
                                    ...
                                    className={`relative cursor-pointer rounded-2xl p-6 flex flex-col justify-between transition-all duration-200 ${
                                        isSelected 
                                        ? 'bg-white dark:bg-slate-800 border-2 border-blue-500 shadow-xl scale-[1.02] z-10' 
                                        : 'bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-700 shadow-sm'
                                    }`}
                                >
                                    {badge && (
                                    ...
                                    <div className={`w-full h-10 rounded-lg flex items-center justify-center font-bold text-sm transition-colors ${isSelected ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400'}`}>
                                        {isSelected ? '선택됨' : '선택하기'}
                                    </div>
                                </div>
                            );
                    ...
                    <div className="text-center max-w-md mx-auto">
                        {user ? (
                             <button
                                ...
                                className="w-full py-4 text-lg font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-colors disabled:bg-slate-300 dark:disabled:bg-slate-700 disabled:cursor-not-allowed shadow-lg flex items-center justify-center gap-2"
                            >
                                {isLoading ? '결제창 로딩 중...' : '선택한 플랜 시작하기'} 
                                {!isLoading && <RocketIcon className="w-5 h-5"/>}
                            </button>
                        ) : (
                             <button
